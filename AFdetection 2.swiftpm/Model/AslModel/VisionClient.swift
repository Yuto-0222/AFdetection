...
class VisionClient: ObservableObject {
    ...
    // デリゲートを持たせるためのプロパティ
    weak var delegate: HandSignDetectorDelegate?
    ...
    var currentGesture: HandGesture = .unknown {
        ...
        didSet {
            delegate?.visionClient(self, didRecognize: currentGesture)
        }
        
    }
    
    // デリゲートを初期化
    init(delegate: HandSignDetectorDelegate? = nil) {
    ...
    func processObservations(_ observations: [VNRecognizedPointsObservation]) {
        ...
        if
            wristToIndexTip < wristToIndexPIP &&
                ...
                wristToRingTip < wristToRingPIP && wristToLittleTip < wristToLittlePIP && wristTothumbTIP > wristTothumbcmc && wristToIndexPIP < wristTothumbTIP {
