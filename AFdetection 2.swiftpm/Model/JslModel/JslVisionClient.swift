...
class JslVisionClient: ObservableObject {
    ...
    // デリゲートを持たせるためのプロパティ
    weak var delegate: JslDetectorDelegate?
    
    // デリゲートメソッドに渡す用のHandGestureプロパティ
    //CSLの指文字を格納
    var jslcurrentGesture: JslHandGesture = .unknown {
        
        //currentGestureが変更された時に実行される
        didSet {
            delegate?.jslvisionClient(self, didRecognize: jslcurrentGesture)
        }
        
    }
    
    // デリゲートを初期化
    init(delegate: JslDetectorDelegate? = nil) {
