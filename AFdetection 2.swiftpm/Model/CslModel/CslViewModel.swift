...
class CslViewModel: NSObject, ObservableObject, AVCaptureVideoDataOutputSampleBufferDelegate, CslDetectorDelegate {
    
    let cslVisionClient: CslVisionClient
    
    // AVCaptureSessionのインスタンス生成
    private let session = AVCaptureSession()
    private var delegate: CslDetectorDelegate?
    
    var frameCounter: Int = 0
    let handPosePredictionInterval: Int = 30
    var currentHandPoseObservation: VNHumanHandPoseObservation?

    @Published var cslcurrentGesture: CslVisionClient.CslHandGesture = .unknown
    
    //SwiftHandPoseClassifierのインスタンス生成
    //SwiftHandPoseClassifierを初期化
    let model = try? CslHandPoseClassifier(configuration: MLModelConfiguration())
    
    override init() {
        cslVisionClient = CslVisionClient()
        super.init()
        cslVisionClient.delegate = self
        do {
    ...
    func cslvisionClient(_ cslVisionClient: CslVisionClient, didRecognize gesture: CslVisionClient.CslHandGesture) {
        ...
        func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
            ...
            //cslVisionClient.processObservations(observations)
        }
        ...
        do {
            ...
            if confidence > 0.7 {
                DispatchQueue.main.async { [self] in
                    switch label {
                         case"S": cslVisionClient.cslcurrentGesture = .S
                         case"W": cslVisionClient.cslcurrentGesture = .W
                         case"I": cslVisionClient.cslcurrentGesture = .I
                         case"F": cslVisionClient.cslcurrentGesture = .F
                         case"T": cslVisionClient.cslcurrentGesture = .T
                    default:
                        break
}
                }
