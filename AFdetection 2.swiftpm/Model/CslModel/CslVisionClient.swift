...
class CslVisionClient: ObservableObject {
    ...
    // デリゲートを持たせるためのプロパティ
    weak var delegate: CslDetectorDelegate?
    
    // デリゲートメソッドに渡す用のHandGestureプロパティ
    //CSLの指文字を格納
    var cslcurrentGesture: CslHandGesture = .unknown {
        
        //currentGestureが変更された時に実行される
        didSet {
            delegate?.cslvisionClient(self, didRecognize: cslcurrentGesture)
        }
        
    }
    
    // デリゲートを初期化
    init(delegate: CslDetectorDelegate? = nil) {
