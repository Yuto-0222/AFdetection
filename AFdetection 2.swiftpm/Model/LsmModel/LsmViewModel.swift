...
class LsmViewModel: NSObject, ObservableObject, AVCaptureVideoDataOutputSampleBufferDelegate, LsmDetectorDelegate {
    
    let lsmVisionClient: LsmVisionClient
    ...
    private var delegate: LsmDetectorDelegate?
    ...
    @Published var lsmcurrentGesture: LsmVisionClient.LsmHandGesture = .unknown
    
    override init() {
        lsmVisionClient = LsmVisionClient()
        super.init()
        lsmVisionClient.delegate = self
        do {
    ...
    func lsmvisionClient(_ lsmVisionClient: LsmVisionClient, didRecognize gesture: LsmVisionClient.LsmHandGesture) {
        ...
        func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
            ...
            //lsmVisionClient.processObservations(observations)
        }
        ...
        do {
            ...
            if confidence > 0.7 {
                DispatchQueue.main.async { [self] in
                    switch label {
                    case "S":lsmVisionClient.lsmcurrentGesture = .S
                    case "W":lsmVisionClient.lsmcurrentGesture = .W
                    case "I":lsmVisionClient.lsmcurrentGesture = .I
                    case "F":lsmVisionClient.lsmcurrentGesture = .F
                    case "T":lsmVisionClient.lsmcurrentGesture = .T
                    default : break
                    }
                }
            }
