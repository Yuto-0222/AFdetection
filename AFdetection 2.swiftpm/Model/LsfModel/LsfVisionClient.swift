...
class LsfVisionClient: ObservableObject {
    ...
    // デリゲートを持たせるためのプロパティ
    weak var delegate: LsfDetectorDelegate?
    
    // デリゲートメソッドに渡す用のHandGestureプロパティ
    //CSLの指文字を格納
    var lsfcurrentGesture: LsfHandGesture = .unknown {
        
        //currentGestureが変更された時に実行される
        didSet {
            delegate?.lsfvisionClient(self, didRecognize: lsfcurrentGesture)
        }
        
    }
    
    // デリゲートを初期化
    init(delegate: LsfDetectorDelegate? = nil) {
