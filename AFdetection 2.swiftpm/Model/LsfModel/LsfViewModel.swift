...
class LsfViewModel: NSObject, ObservableObject, AVCaptureVideoDataOutputSampleBufferDelegate, LsfDetectorDelegate {
    
    let lsfVisionClient: LsfVisionClient
    
    //SwiftHandPoseClassifierのインスタンス生成
    //SwiftHandPoseClassifierを初期化
    let model = try? SslHandPoseClassifier(configuration: MLModelConfiguration())
    
    //指文字取得に関するプロパティ
    var frameCounter: Int = 0
    let handPosePredictionInterval: Int = 30
    var currentHandPoseObservation: VNHumanHandPoseObservation?
    var sslcurrentHandPoseObservation: VNHumanHandPoseObservation?
    
    // AVCaptureSessionのインスタンス生成
    private let session = AVCaptureSession()
    private var delegate: LsfDetectorDelegate?

    @Published var lsfcurrentGesture: LsfVisionClient.LsfHandGesture = .unknown
    
    override init() {
        lsfVisionClient = LsfVisionClient()
        super.init()
        lsfVisionClient.delegate = self
        do {
    ...
    func lsfvisionClient(_ lsfVisionClient: LsfVisionClient, didRecognize gesture: LsfVisionClient.LsfHandGesture) {
        ...
        func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
            ...
            //lsfVisionClient.processObservations(observations)
        }
        ...
        do {
            ...
            if confidence > 0.7 {
                DispatchQueue.main.async { [self] in
                    switch label {
                    case "S":lsfVisionClient.lsfcurrentGesture = .S
                    case "W":lsfVisionClient.lsfcurrentGesture = .W
                    case "I":lsfVisionClient.lsfcurrentGesture = .I
                    case "F":lsfVisionClient.lsfcurrentGesture = .F
                    case "T":lsfVisionClient.lsfcurrentGesture = .T
                    default : break
                    }
                }
            }
